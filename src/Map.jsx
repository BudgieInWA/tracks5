...
class Map extends React.Component {
  ...
  render() {
    ...
    const box = _.map(
      [-1 + this.state.viewOffset.x, -1 + this.state.viewOffset.y, 2, 2],
      (c) => c * zoomScale
    );
    ...
    return (
      <HexGrid width="100%" height="100%" viewBox={box.join(' ')}>
        <Layout size={{ x: 1, y: 1 }}>
          <Terrain
            terrain={terrain}
            handlersFactory={finishFactory({ targetName: 'tile', dispatch })}
          />
          <Tracks
            tracks={tracks}
            handlersFactory={finishFactory({ targetName: 'track', dispatch })}
          />
          <Buildings
            buildings={buildings}
            handlersFactory={finishFactory({ targetName: 'building', dispatch })}
          />
          <Trains
            trains={trains}
            handlersFactory={finishFactory({ targetName: 'train', dispatch })}
          />

          <g className="tool">
            {tool.hexes.length && <Path hexes={tool.hexes} />}
            {currentTool.renderMap && (
              <currentTool.renderMap {...{ tool, terrain, tracks, trains, buildings }} />
            )}
          </g>
        </Layout>
      </HexGrid>
    ...
    return (
      <g className={classNames('buildings', { touchable: handlersFactory !== nullFactory })}>
        {_.map(buildings, (building) => {
          ...
          return <BuildingComp key={building.name} {...building} {...handlersFactory(building)} />;
        })}
      </g>
    );
  }
  ...
  render() {
    ...
    return (
      <g className={classNames('trains', { touchable: handlersFactory !== nullFactory })}>
        {_.map(trains, (train) => (
          <TrainCar key={train.name} {...train} {...handlersFactory(train)} />
        ))}
      </g>
    );
  }
